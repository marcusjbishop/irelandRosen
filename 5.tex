...
If $a$ is a residue then there are $2=1+\tlegendre{a}{p}$ solutions.
If $a$ is a nonresidue then there are $0=1+\tlegendre{a}{p}$ solutions.
If $p\mid a$ then there is $1=1+\tlegendre{a}{p}$ solution,
namely $x=0$.
...
$1=1+\tlegendre{b^2-4ac}{p}$. Otherwise assume that
$p\nmid b^2-4ac$.
...
$2=1+\tlegendre{b^2-4ac}{p}$ in this situation. Finally
$0=1+\tlegendre{b^2-4ac}{p}$ if there are no solutions to
$y^2\equiv b^2-4ac$.

\item\label{SumLegendre} %4
$\tlegendre{a}{p}=1$ if and only if
...
It follows that $\sum_{a=1}^{p-1}\tlegendre{a}{p}=0$.
...
Next we find the primes $p$ for which $\tlegendre{15}{p}=1$.
Suppose $\tlegendre{3}{p}=1=\tlegendre{5}{p}$.
...
\end{itemize}
Suppose now that $\tlegendre{3}{p}=-1=\tlegendre{5}{p}$.
...
\end{itemize}
In summary, we have $\tlegendre{15}{p}$ if $p\equiv
...
with $\tlegendre{a_i}{p_i}=1$ for all $1\le i\le k-1$ and
$\tlegendre{a_k}{p_k}=-1$. By the Chinese Remainder Theorem
the system 
...
The prime factorization of $D$ is $p_1p_2\cdots p_k$
where $p_1,p_2,\ldots,p_k$ are distinct, odd primes. 
For each $1\le i\le k$ let $b^{+1}_1,b^{+1}_2,\ldots,
b^{+1}_{\frac{p_i-1}{2}},
b^{-1}_1,b^{-1}_2,\ldots,b^{-1}_{\frac{p_i-1}{2}}$ be
integers such that $\dlegendre{b^+_j}{p_i}=1$ and
$\dlegendre{b^-_j}{p_i}=-1$ for all
$1\le j\le\frac{p_i-1}{2}$.
For each $\epsilon\in\left(\epsilon_1,\epsilon_2,\ldots,\epsilon_k\right)
\in\left\{1,-1\right\}^k$
and each $a=\left(a_1,a_2,\ldots,a_k\right)
\in\prod_{i=1}^k\left\{b_1^{\epsilon_1},b_2^{\epsilon_2},
\ldots b_k^{\epsilon_k}\right\}$ there is exactly one solution
$1\le b^{a,\epsilon}\le D-1$ to the system
$\left\{x\equiv a_i\pmod{p_i}\left|
1\le i\le k\rule{0pt}{11pt}\right.\right\}$.
Note that $\dlegendre{b^{a,\epsilon}}{D}
=\epsilon_1\epsilon_2\cdots\epsilon_k$
and that $b^{a,\epsilon}$ ranges over a reduced residue system $\pmod{D}$
as $\epsilon$ and $a$ range over all tuples.
Now since $\epsilon_1\epsilon_2\cdots\epsilon_k$
equals $+1$ for half the tuples $\epsilon$ and $-1$ for the other half,
it follows that $\sum\dlegendre{b^{a,\epsilon}}{D}=0$.

\item %20
The prime factorization of $D$ is $p_1p_2\cdots p_k$
where $p_1,p_2,\ldots,p_k$ are distinct, odd primes. 
Fix a prime $p\ne p_1,p_2,\ldots,p_k$ with $p\equiv 1\pmod{4}$.
Note that
$\textstyle\left(\frac{D-1}{2}\right)\left(\frac{p-1}{2}\right)$
is even since $p\equiv 1\pmod{4}$. This means
that
\begin{equation}\label{DPRecriprocity}
1=\dlegendre{D}{p}\dlegendre{p}{D}.
\end{equation}
We will show that $D$ is a residue $\pmod{p}$
if and only if $p\equiv a_i\pmod{D}$ for some $i$.

Suppose that $D$ is a residue $\pmod{p}$
so that $\tlegendre{D}{p}=1$.
This means that $\tlegendre{p}{D}=1$ by \autoref{DPRecriprocity}.
Further, we know that
$\left(D,p\right)=1$ since $p\ne p_1,p_2,\ldots,p_k$. So $p$ is a unit
$\pmod{D}$. If $p$ were congruent to one of the
$\frac{\varphi\left(D\right)}{2}$ units $b$ with $\tlegendre{b}{D}=-1$
then $\tlegendre{p}{D}$ would be $-1$, a contradiction. Thus
$p$ is congruent $\pmod{D}$ to one of $a_1,a_2,\ldots,
a_{\frac{\varphi\left(D\right)}{2}}$.

Suppose conversely that $p\equiv a_i\pmod{D}$.
Then $\tlegendre{p}{D}=\tlegendre{a_i}{D}=1$
so that $\tlegendre{D}{p}=1$ by \autoref{DPRecriprocity}.

\item %21
$1,4,5,16,17,20$ are the integers $b$ with $1\le b<21$,
$\left(b,21\right)=1$, and $\tlegendre{b}{21}$.
