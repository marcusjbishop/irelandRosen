...
\item Let $\alpha\in\mathbb{F}_{p^n}$
and suppose that $g_\alpha\left(x\right)
=\sum_{i=0}^{n-1}b_ix^i\in\mathbb{F}_p\left[x\right]$
is the minimal polynomial of $\alpha$ over $\mathbb{F}_p$.
Then $0=g_\alpha\left(\alpha\right)=\sum_{i=0}^{n-1}b_i\alpha^i$.
Now for any $0\le j\le n-1$ we have
\[g_\alpha\left(\alpha^{p^j}\right)
=\sum_{i=0}^{n-1}b_i\alpha^{ip^j}
=\left(\sum_{i=0}^{n-1}b_i\alpha^i\right)^{p^j}=0.\]
...
coincides with $g_\alpha\left(x\right)$. We conclude that
