...
is a homomorphisms of $\mathbb{F}_p$-modules.
It follows that $\ker\mathsf{tr}$ is a $\mathbb{F}_p$-submodule
and $\left|\ker\mathsf{tr}\right|=p^{n-1}$
since $\mathsf{tr}$ is surjective.
Therefore $\ker\mathsf{tr}\ne\emptyset$.
\end{enumerate}
...
that $x\mapsto x-x^p$ is a homomorphism
of $\mathbb{F}_p$-modules. The kernel of $x\mapsto x-x^p$
is $\mathbb{F}_p$ so the size of
$M=\left\{x-x^p\mid x\in \mathbb{F}_{p^n}\right\}$ is $p^{n-1}$.
As noted in~\autoref{Tr}
the size of $\ker\mathsf{tr}$ is also $p^{n-1}$.
